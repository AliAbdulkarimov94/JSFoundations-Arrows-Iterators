...
function toCelsius (temperatures) {
  return temperatures.map((temprature) => 
       (temprature - 32 ) * (5/9)
  )
}
...
function hottestDays (temperatures, threshhold) {
    const differentTemps = temperatures.filter((temperature) => {
        return (temperature > threshhold)

    })
    return differentTemps
};
...
const differentTemps = hottestDays(temperatures,threshhold)
const TempsInCel = toCelsius (differentTemps)
logger(TempsInCel)
};
